...
@section('content')
        ...
        <div class="relative w-full sm:w-80">
            ...
            <form method="GET" action="{{ route('admin.report.index') }}" class="flex">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul atau token..."
                    class="input-field !pl-10 !rounded-r-none flex-1">
    ...
    <!-- Filter Tabs -->
    <div x-data="filterTabs()" class="flex gap-2 overflow-x-auto pb-2 mb-6">
        @foreach(['Semua', 'Pending', 'Diproses', 'Selesai', 'Ditolak'] as $filter)
            <a href="{{ request()->fullUrlWithQuery(['status' => $filter === 'Semua' ? null : $filter, 'search' => request('search') ?: null]) }}"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-center gap-2 mb-2">
                                <span
                                    class="badge-{{ Str::lower($report->category->weight_level) }}">{{ $report->category->weight_level }}</span>
                                <span class="badge-{{ Str::lower($report->status) }}">{{ $report->status }}</span>
                                @if($report->is_anonymous)
                                    <span class="badge bg-slate-100 text-slate-500">Anonim</span>
                                @endif
                            </div>
                        ...
                        <div class="text-left sm:text-right flex-shrink-0 space-y-1">
                            ...
                            <p class="text-xs text-slate-500">
                                {{ $report->is_anonymous ? '🛡️ Pelapor Anonim' : '👤 ' . $report->user->username }}</p>
                        </div>
                    </div>
                </a>
            @endforeach
